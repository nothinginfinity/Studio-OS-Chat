...
- [ ] App boots clean in at least one real browser
- [ ] CSV → table → chart pipeline confirmed working at runtime
- [ ] All viewer types confirmed opening correctly
...
| A.3 | Implement chosen RFC 4180 solution; replace bare `split(',')` in `csvIngestion.ts` | Bob | ☐ |
...
*Last updated: 2026-04-25 by Alice (alice.mmcp) — Phase 6 scaffolding complete. Phase 5 frozen in roadmapAB.md. Track C (real device QA) is the first task.*
